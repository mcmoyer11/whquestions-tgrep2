...
normed$Wh = as.factor(normed$Wh)
normed$paraphrase = as.factor(normed$paraphrase)
normed$QuestionType = as.factor(normed$QuestionType)
normed$VerbLemma = as.factor(normed$VerbLemma)

# Set the reference levels
contrasts(normed$paraphrase)
contrasts(normed$Wh)

contrasts(normed$Wh) = cbind("how.vs.when"=c(0,1,0,0,0,0),"what.vs.when"=c(1,0,0,0,0,0),
...
contrasts(normed$paraphrase) = cbind("a.vs.every"=c(1,0,0),"the.vs.every"=c(0,0,1))
