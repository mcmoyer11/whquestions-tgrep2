...
agr = test %>%
  filter(MatrixVerb %in% c("know","")) %>%
  group_by(MatrixVerb,ModalPresent) %>%
  summarize(mean_rating = mean(rating), CILow = ci.low(rating), CIHigh = ci.high(rating)) %>%
  mutate(YMin = mean_rating - CILow, YMax = mean_rating + CIHigh) %>%
  mutate(fct_reorder(MatrixVerb,mean_rating)) %>%
  drop_na()

ggplot(agr,aes(x=MatrixVerb, y=mean_rating, fill=MatrixVerb)) + # alpha=ModalPresent,
  geom_bar(position="dodge",stat="identity") +
...
# ggsave("../graphs/test_everything_ModxWh.pdf")
...
unique(normed$MatrixVerb)

normed$VerbLemma = as.factor(
  ifelse(normed$MatrixVerb %in% c("believe","believed"),"believe",
         ifelse(normed$MatrixVerb %in% c("thinking","thinks","thought","think"),"think",
                ifelse(normed$MatrixVerb %in% c("saw","see","seeing"),"see",
                       ifelse(normed$MatrixVerb %in% c("know","knows","knew","known","knowing"),"know",
                              ifelse(normed$MatrixVerb %in% c("realize","realized"),"realize",
                                     ifelse(normed$MatrixVerb %in% c("notice","noticed"),"notice",
                                            ifelse(normed$MatrixVerb %in% c("wonder","wondered","wondering"),"wonder",
                                                   ifelse(normed$MatrixVerb %in% c("ask","asking","asked"),"ask",
                                                          ifelse(normed$MatrixVerb %in% c("remember","remembered","remembering"),"remember",
                                                                 ifelse(normed$MatrixVerb %in% c("is","are","was","were","am"),"be", 
                                                                        ifelse(normed$MatrixVerb %in% c("say","said","saying"),"say",
                                                   "other"))))))))))))


agr = normed %>%
  filter(VerbLemma %in% c("know","see","wonder","realize","think","notice","ask","remember","say","believe")) %>%
  group_by(paraphrase,VerbLemma) %>%
  summarize(mean_rating = mean(normed_rating), CILow = ci.low(normed_rating), CIHigh = ci.high(normed_rating)) %>%
  mutate(YMin = mean_rating - CILow, YMax = mean_rating + CIHigh) %>%
  drop_na()
View(agr)

ggplot(agr,aes(x=VerbLemma, y=mean_rating, fill=paraphrase)) +
  geom_bar(position="dodge",stat="identity") +
  geom_errorbar(aes(ymin=YMin,ymax=YMax), width=.25,position=position_dodge(0.9)) +
  xlab("Matrix Verb") +
  ylab("Mean rating") +
...
ex = normed %>%
  filter(tgrep_id %in% c("132641:45")) %>%
  group_by(paraphrase) %>%
  summarize(mean_rating = mean(normed_rating), sd = sd(normed_rating))
View(ex)  
