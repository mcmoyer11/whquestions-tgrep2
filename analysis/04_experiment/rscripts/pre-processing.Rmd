...
```{r comments and demographic information}
...
```{r TODO: look at first response on practice trials only}
prac_agr = practice %>%
...
ggplot(normed,aes(x=normed_rating)) +
...
agr = normed %>%
  group_by(paraphrase,MatrixVerb) %>%
  summarize(mean_rating = mean(normed_rating), CILow = ci.low(normed_rating), CIHigh = ci.high(normed_rating)) %>%
  mutate(YMin = mean_rating - CILow, YMax = mean_rating + CIHigh) %>%
  drop_na()
View(agr)

ggplot(agr,aes(x=MatrixVerb, y=mean_rating, fill=paraphrase)) +
  geom_bar(position="dodge",stat="identity") +
  geom_errorbar(aes(ymin=YMin,ymax=YMax), width=.25,position=position_dodge(0.9)) +
  xlab("Matrix Verb") +
  ylab("Mean rating") +
